#define NewAnalyzer_dacapo_cxx
#include "NewAnalyzer_dacapo.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <set>
...
void NewAnalyzer_dacapo::Loop()
{
  //   In a ROOT session, you can do:
  //      root> .L NewAnalyzer_dacapo.C
  //      root> NewAnalyzer_dacapo t
 ...
 float entered=0;
 float exitingFirstFilter=0;
 float exitingSecondFilter=0;
 float score_eta;
 float score_pt;
 float score_deltaB;
 double Occurrency;

 //matrice 3D per istogrammi pt-eta
 double Matrix_eta_pt[500] [500]; //Non riesco ad allocare dinamicamente questa matrice e sono stanco 
 double Matrix_eta_deltaB[500] [500]; 
 double Matrix_pt_deltaB[500] [500]; 
 double* E_Tab_eta= new double [Nbin_eta+1];
 double* E_Tab_pt= new double [Nbin_pt+1];

  //raccoglitori per histo eta e pt 
...
for(int i=0; i<500;++i)
	  {for(int j=0; j<500;++j)
	    {Matrix_eta_pt[i] [j]=0;
	     Matrix_eta_deltaB[i] [j]=0;
	     Matrix_pt_deltaB[i] [j]=0;
	    if(i<=Nbin_eta) E_Tab_eta[i]=0;
	     if(j<=Nbin_pt)  E_Tab_pt[j]=0;
	    }
	 }
...
for (Long64_t jentry=0; jentry<nentries;jentry++) {  
	...
	if(Genp_particleId_mother->at(i) == 36 || Genp_particleId_mother->at(i) == 35 || Genp_particleId_mother->at(i) == 25){
	  mother_mass = Genp_m_mother->at(i);      //numeri riferiti al codice di catalogazione degli Higgs riportati nel PDG (Cap. 34 Montecarlo)
...
//    histos["hist_mass_gen"]->Fill(mother_mass, PU_Weight);
      ...
      if(mother_mass> Min_mass && mother_mass< Max_mass  )
	goodevent = true;
      //else
...
//Si salta l'evento se if(metfilter==0) (è una tecnicaglia)      
       if(metfilter == 0) continue; 
      
       metfiltrati++;
	

//Efficienza Filtro Iso in funzione di pt e eta

      for  (unsigned i=0; i<Mu_isTightMu->size(); i++) {
        hist_pt_before_eta_pt_filter->Fill(Mu_pt->at(i),PU_Weight);
    
	hist_eta_before_eta_pt_filter->Fill(Mu_eta->at(i),PU_Weight);
	hist_eta_pt_before_eta_pt_filter->Fill(Mu_eta->at(i),Mu_pt->at(i),PU_Weight);
	hist_px_before_eta_pt_filter->Fill(Mu_px->at(i),PU_Weight);
	hist_py_before_eta_pt_filter->Fill(Mu_py->at(i),PU_Weight);
	hist_pz_before_eta_pt_filter->Fill(Mu_pz->at(i),PU_Weight);
	hist_en_before_eta_pt_filter->Fill(Mu_en->at(i),PU_Weight);
	hist_phi_before_eta_pt_filter->Fill(Mu_phi->at(i),PU_Weight);
	hist_charge_before_eta_pt_filter->Fill(Mu_charge->at(i),PU_Weight);

	///////////////////////////////////////////////////////////////////////////////
	////////////////////////// Reco Acceptance ///////////////////////////////////
//If the muons are Tight they must have:
//pT > 26GeV and |eta| < 2.4 (eta è la pseudorapidità)
//relative isolation < 0.25
//at least one muon firing the IsoMu24 or the IsoTkMu24 trigger

	if(fabs(Mu_eta->at(i))<2.4 && Mu_pt->at(i) > 26.){
	
	hist_pt_before_iso_filter->Fill(Mu_pt->at(i),PU_Weight);
	hist_eta_before_iso_filter->Fill(Mu_eta->at(i),PU_Weight);
	hist_eta_pt_before_iso_filter->Fill(Mu_eta->at(i),Mu_pt->at(i),PU_Weight);
	hist_px_before_iso_filter->Fill(Mu_px->at(i),PU_Weight);
	hist_py_before_iso_filter->Fill(Mu_py->at(i),PU_Weight);
	hist_pz_before_iso_filter->Fill(Mu_pz->at(i),PU_Weight);
	hist_en_before_iso_filter->Fill(Mu_en->at(i),PU_Weight);
	hist_phi_before_iso_filter->Fill(Mu_phi->at(i),PU_Weight);
	hist_charge_before_iso_filter->Fill(Mu_charge->at(i),PU_Weight);
...
//////////////isolation cut
	    if(deltaB_corrected < 0.25){
	      ...
	      hist_pt_after_iso_filter->Fill(Mu_pt->at(i),PU_Weight);
	      hist_eta_after_iso_filter->Fill(Mu_eta->at(i),PU_Weight);
	      hist_eta_pt_after_iso_filter->Fill(Mu_eta->at(i),Mu_pt->at(i),PU_Weight);
	      hist_px_after_iso_filter->Fill(Mu_px->at(i),PU_Weight);
	      hist_py_after_iso_filter->Fill(Mu_py->at(i),PU_Weight);
	      hist_pz_after_iso_filter->Fill(Mu_pz->at(i),PU_Weight);
	      hist_en_after_iso_filter->Fill(Mu_en->at(i),PU_Weight);
	      hist_phi_after_iso_filter->Fill(Mu_phi->at(i),PU_Weight);
	      hist_charge_after_iso_filter->Fill(Mu_charge->at(i),PU_Weight);
	    }
	  }//end isTight
  ...
  ///////////////////////////////////////////////////////////////////////////////
	...
	histEff_eta_ptFilter_pt->Divide(hist_pt_before_iso_filter,hist_pt_before_eta_pt_filter,1,1);
        histEff_eta_ptFilter_eta->Divide(hist_eta_before_iso_filter,hist_eta_before_eta_pt_filter,1,1);
	histEff_eta_ptFilter_eta_pt->Divide(hist_eta_pt_before_iso_filter,hist_eta_pt_before_eta_pt_filter,1,1);
        histEff_eta_ptFilter_px->Divide(hist_px_before_iso_filter,hist_px_before_eta_pt_filter,1,1);
        histEff_eta_ptFilter_py->Divide(hist_py_before_iso_filter,hist_py_before_eta_pt_filter,1,1);
        histEff_eta_ptFilter_pz->Divide(hist_pz_before_iso_filter,hist_pz_before_eta_pt_filter,1,1);
        histEff_eta_ptFilter_en->Divide(hist_en_before_iso_filter,hist_en_before_eta_pt_filter,1,1);
        histEff_eta_ptFilter_phi->Divide(hist_phi_before_iso_filter,hist_phi_before_eta_pt_filter,1,1);
        histEff_eta_ptFilter_charge->Divide(hist_charge_before_iso_filter,hist_charge_before_eta_pt_filter,1,1);

	histEff_IsoFilter_pt->Divide(hist_pt_after_iso_filter,hist_pt_before_iso_filter,1,1);
        histEff_IsoFilter_eta->Divide(hist_eta_after_iso_filter,hist_eta_before_iso_filter,1,1);
	histEff_IsoFilter_eta_pt->Divide(hist_eta_pt_after_iso_filter,hist_eta_pt_before_iso_filter,1,1);
        histEff_IsoFilter_px->Divide(hist_px_after_iso_filter,hist_px_before_eta_pt_filter,1,1);
        histEff_IsoFilter_py->Divide(hist_py_after_iso_filter,hist_py_before_iso_filter,1,1);
        histEff_IsoFilter_pz->Divide(hist_pz_after_iso_filter,hist_pz_before_iso_filter,1,1);
        histEff_IsoFilter_en->Divide(hist_en_after_iso_filter,hist_en_before_iso_filter,1,1);
        histEff_IsoFilter_phi->Divide(hist_phi_after_iso_filter,hist_phi_before_iso_filter,1,1);
        histEff_IsoFilter_charge->Divide(hist_charge_after_iso_filter,hist_charge_before_iso_filter,1,1);
   ...
   //   histos["eventWeight_PU_Muons"]->Fill(PU_Weight);
        ...
        hist_mass_inclusive_pre_MCut->Fill(tight_dimuon.M(), PU_Weight);
	hist_p_inclusive_pre_MCut->Fill(tight_dimuon.P(), PU_Weight);
	hist_pt_inclusive_pre_MCut->Fill(tight_dimuon.Pt(), PU_Weight);
	hist_pz_inclusive_pre_MCut->Fill(tight_dimuon.Pz(), PU_Weight);
	hist_eta_inclusive_pre_MCut->Fill(tight_dimuon.Eta(), PU_Weight);
	hist_phi_inclusive_pre_MCut->Fill(tight_dimuon.Phi(), PU_Weight);
   
    //select only pairs above 130 GeV
	if(tight_dimuon.M() > 200 && tight_dimuon.M()<400){
...
//A che minchia serve questo loop? Ma poi non dovrebbe essere riazzerato ad ogni ciclo?
	for(int m=0; m<NpfMet; ++m){
		My_MEt_pt+= Met_pt_pfmet->at(m);
		My_MEt_phi += Met_phi_pfmet->at(m);
		My_MEt_eta  += Met_eta_pfmet->at(m);
  ...
  //Loop over jets
	for(int j=0;j<NpfJets;++j){
		jet.SetPxPyPzE(Jet_px_pfjet->at(j),Jet_py_pfjet->at(j),Jet_pz_pfjet->at(j),Jet_en_pfjet->at(j));
		//jet cleaning
	...
	CleanJetsFromMuons(jets2,bdisc2,the_twomuons,0.4);
		...
		hist_bdisc->Fill(bdisc[i],PU_Weight);
	       if(bdisc[i] >= Btag_disc_cut){
			delta_R_bjet_dimuon[Nbjet] = jets[i].DeltaR(tight_dimuon);
			bjet_pt[Nbjet] = jets[i].Pt();
			bjet_eta[Nbjet] = jets[i].Eta();
			Nbjet++;
		}
	...
	if(Nbjet != 0){//ovvero se ho un evento btaggato
		delta_pt_mupair_1bjet = tight_dimuon.Pt() - bjet_pt[0];
		delta_eta_mupair_1bjet = tight_dimuon.Eta() - bjet_eta[0];
		hist_N_jet_btag_for_event->Fill(Nbjet,PU_Weight	);
		hist_N_jet2_btag_for_event->Fill(Nbjet,PU_Weight);
		hist_pt_MEt_btag_event->Fill(My_MEt_pt,PU_Weight);
		hist_energy_MEt_btag_event->Fill(My_MEt_energy,PU_Weight);
	}
	else {hist_pt_MEt_non_btag_event->Fill(My_MEt_pt,PU_Weight);
		hist_energy_MEt_non_btag_event->Fill(My_MEt_energy,PU_Weight);
	     }
...
//Finito Jet
	...
	//categorization
        ...
        hist_mass_inclusive_post_MCut->Fill(tight_dimuon.M(), PU_Weight);
	hist_p_inclusive_post_MCut->Fill(tight_dimuon.P(), PU_Weight);
	hist_pt_inclusive_post_MCut->Fill(tight_dimuon.Pt(), PU_Weight);
	hist_pz_inclusive_post_MCut->Fill(tight_dimuon.Pz(), PU_Weight);
	hist_eta_inclusive_post_MCut->Fill(tight_dimuon.Eta(), PU_Weight);
	hist_phi_inclusive_post_MCut->Fill(tight_dimuon.Phi(), PU_Weight);
...
//GRAFICO FILTRI ETA PT
	//Riempio il matricione
	for(int etamax=0; etamax<Nbin_eta;++etamax)
	  {for(int ptmin=0; ptmin<Nbin_pt;++ptmin)
	   {for(int deltaBmax=0;deltaBmax<Nbin_deltaB;++deltaBmax)
	     {entered=0;
	     exitingFirstFilter=0;
	     exitingSecondFilter=0;
	     score_eta=(etamax+1)*binMax_eta/Nbin_eta;
	     score_pt=(ptmin+1)*binMax_pt/Nbin_pt;
	     score_deltaB=(deltaBmax+1)*binMax_deltaB/Nbin_deltaB;
	     for  (unsigned i=0; i<Mu_isTightMu->size(); i++) 
	      { ++entered;
		if(fabs(Mu_eta->at(i))<score_eta && Mu_pt->at(i) > score_pt)
		{++exitingFirstFilter;
		...
		if(Mu_isTightMu->at(i) > 0){
			...
			//////////////isolation cut
	    		if(deltaB_corrected < score_deltaB)
			{++exitingSecondFilter;
	   		 }
	  		}//end isTight
             ...
             if(deltaBmax>=(0.25*(Nbin_deltaB/binMax_deltaB)-1) && deltaBmax<(0.25*(Nbin_deltaB/binMax_deltaB)))
	  		 {if(entered==0  && exitingFirstFilter!= 0){std::cout<<"c'è un bug!\n";}
	     		 else if (entered==0  && exitingFirstFilter== 0)Matrix_eta_pt[etamax][ptmin]+=0;
			 else Matrix_eta_pt[etamax][ptmin]+=(exitingFirstFilter/entered);
			 }     
	     if(ptmin>=(26*(Nbin_pt/binMax_pt)-1) && ptmin<(26*(Nbin_pt/binMax_pt)))
			 {if(exitingFirstFilter==0  && exitingSecondFilter!= 0){std::cout<<"c'è un bug!\n";}
	     		 else if (exitingFirstFilter==0  && exitingSecondFilter== 0)Matrix_eta_deltaB[etamax][deltaBmax]+=0;
		 	 else Matrix_eta_deltaB[etamax][deltaBmax]+=(exitingSecondFilter/exitingFirstFilter);
			 }
		if(etamax>=(2.4*(Nbin_eta/binMax_eta)-1) && etamax<(2.4*(Nbin_eta/binMax_eta)))
	   		{if(exitingFirstFilter==0  && exitingSecondFilter!= 0){std::cout<<"c'è un bug!\n";}
	     		 else if (exitingFirstFilter==0  && exitingSecondFilter== 0)Matrix_pt_deltaB[ptmin][deltaBmax]+=0;
		  	 else Matrix_pt_deltaB[ptmin][deltaBmax]+=(exitingSecondFilter/exitingFirstFilter);
	 		}	
	     }
	    }
	  }
	...
	//riempio gli histo eta_pt, eta_for_pt26, pt_for_eta24 e i raccoglitori per gli histo pt e eta   
	for(int etamax=0; etamax<Nbin_eta;++etamax)
	  {for(int ptmin=0; ptmin<Nbin_pt;++ptmin)
	   {for(int deltaBmax=0;deltaBmax<Nbin_deltaB;++deltaBmax)
 	    {score_eta=(etamax+1)*binMax_eta/Nbin_eta;
	     ...
	     score_deltaB=(deltaBmax+1)*binMax_deltaB/Nbin_deltaB;
             if(deltaBmax>=(0.25*(Nbin_deltaB/binMax_deltaB)-1) && deltaBmax<(0.25*(Nbin_deltaB/binMax_deltaB)))
	       {Occurrency=Matrix_eta_pt[etamax][ptmin]/metfiltrati;
	    	 if(Occurrency>1) std::cout<<"non va bene eta_pt"<< etamax<<"-"<< ptmin<<std::endl;
		//std::cout<<"Entro ora in eta-pt "<< etamax<<"-"<<std::endl;
	     	 hist_eta_pt->SetBinContent(etamax,ptmin,Occurrency);
		
	    	 if(score_eta>=2.4 && score_eta<2.4+(binMax_eta/Nbin_eta))//cioè  score_eta >=2.4 e score_eta-1bin<=2.4
			{Occurrency=Matrix_eta_pt[etamax][ptmin]/(metfiltrati); //Riempio l'istogramma pt con eta fissato a 2.4
     	  	 	 hist_pt_for_eta24->SetBinContent(ptmin+1,Occurrency);}
	     	if(score_pt>=26 && score_pt<26+(binMax_pt/Nbin_pt))//Riempio l'istogramma eta con pt fissato a 26
		 	{Occurrency=Matrix_eta_pt[etamax][ptmin]/(metfiltrati);
     	  	 	hist_eta_for_pt26->SetBinContent(etamax+1,Occurrency);
			}
	        E_Tab_eta[etamax]+=Matrix_eta_pt[etamax][ptmin];
	        E_Tab_pt[ptmin]+=Matrix_eta_pt[etamax][ptmin];
	       }
	     if(ptmin>=(26*(Nbin_pt/binMax_pt)-1) && ptmin<(26*(Nbin_pt/binMax_pt)))
		{//std::cout<<"Entro ora in eta-deltaB "<< score_pt<<"-"<<std::endl;
		 Occurrency=Matrix_eta_deltaB[etamax][deltaBmax]/metfiltrati;
		 if(Occurrency>1) std::cout<<"non va bene eta_deltaB"<< etamax<<"-"<< deltaBmax<<std::endl;
	     	 hist_eta_deltaB->SetBinContent(etamax,deltaBmax,Occurrency);
	        }
	     if(etamax>=(2.4*(Nbin_eta/binMax_eta)-1) && etamax<(2.4*(Nbin_eta/binMax_eta)))
		{//std::cout<<"Entro ora in PT-deltaB "<< score_eta<<"-"<<std::endl;
		 Occurrency=Matrix_eta_deltaB[ptmin][deltaBmax]/metfiltrati;
		 if(Occurrency>1) std::cout<<"non va bene pt_deltaB"<< ptmin<<"-"<< deltaBmax<<std::endl;
	     	 hist_pt_deltaB->SetBinContent(ptmin,deltaBmax,Occurrency);
	        }
	...
	for(int etamax=0; etamax<Nbin_eta;++etamax){
	  Occurrency=E_Tab_eta[etamax]/(metfiltrati*Nbin_pt);
	  hist_eta->SetBinContent(etamax+1,Occurrency);
	  if  (Occurrency>1)std::cout<<"non ci siamo etamax\n"<<etamax<<"\n";
	}
	for(int ptmin=0; ptmin<Nbin_pt;++ptmin){
          Occurrency=E_Tab_pt[ptmin]/(metfiltrati*Nbin_eta);
     	  hist_pt->SetBinContent(ptmin+1,Occurrency);
	  if  (Occurrency>1)std::cout<<"non ci siamo ptmin\n"<<ptmin<<"\n";
	  }
...
void NewAnalyzer_dacapo::BookHistos() {// here is where to book histos -> Function called by Init
  ...
  signaltree_->Branch("Met_Pt",&My_MEt_pt);
...
void NewAnalyzer_dacapo::Draw() { // To be called After the Loop to look at plots
  TString file_name = "./Out_" + sel_dataset + "_200<M<400_test.root";
...
*/
  outputFile->cd("/");
  //outputFile->mkdir("Percentuale Filtrati");
//  outputFile->cd("Percentuale Filtrati");

  	hist_eta_pt->Write();
  	hist_eta_for_pt26 -> Write();
  	hist_pt_for_eta24->Write();
 	 hist_eta-> Write();
 	 hist_pt->Write();
	hist_eta_deltaB->Write();
	hist_pt_deltaB->Write();

  outputFile->cd("/");
 // outputFile->mkdir("Before eta-pt");
  //outputFile->cd("Before eta-pt");
        hist_pt_before_eta_pt_filter->Write();
	hist_eta_before_eta_pt_filter->Write();
	hist_eta_pt_before_eta_pt_filter->Write();
	hist_pt_before_eta_pt_filter->Write();
	hist_eta_before_eta_pt_filter->Write();
	hist_eta_pt_before_eta_pt_filter->Write();
	hist_px_before_eta_pt_filter->Write();
	hist_py_before_eta_pt_filter->Write();
	hist_pz_before_eta_pt_filter->Write();
	hist_en_before_eta_pt_filter->Write();
	hist_phi_before_eta_pt_filter->Write();
	hist_charge_before_eta_pt_filter->Write();

  outputFile->cd("/");
 // outputFile->mkdir("Post eta-pt Before Iso");
 // outputFile->cd("Post eta-pt Before Iso");

	hist_pt_before_iso_filter->Write();
	hist_eta_before_iso_filter->Write();
	hist_eta_pt_before_iso_filter->Write();
	hist_px_before_iso_filter->Write();
	hist_py_before_iso_filter->Write();
	hist_pz_before_iso_filter->Write();
	hist_en_before_iso_filter->Write();
	hist_phi_before_iso_filter->Write();
	hist_charge_before_iso_filter->Write();
  outputFile->cd("/");
 // outputFile->mkdir("After Iso");
 // outputFile->cd("After Iso");
	hist_pt_after_iso_filter->Write();
	hist_eta_after_iso_filter->Write();
	hist_eta_pt_after_iso_filter->Write();
	hist_px_after_iso_filter->Write();
	hist_py_after_iso_filter->Write();
	hist_pz_after_iso_filter->Write();
	hist_en_after_iso_filter->Write();
	hist_phi_after_iso_filter->Write();
	hist_charge_after_iso_filter->Write();
 // outputFile->cd("/");  
  //outputFile->mkdir("Efficienza Filtro Eta-Pt");
  //outputFile->cd("Efficienza Filtro Eta-Pt");
  	histEff_eta_ptFilter_pt->Write();
        histEff_eta_ptFilter_eta->Write();
 	histEff_eta_ptFilter_eta_pt->Write();
  	histEff_eta_ptFilter_px->Write();
        histEff_eta_ptFilter_py->Write();
  	histEff_eta_ptFilter_pz->Write();
        histEff_eta_ptFilter_en->Write();
  	histEff_eta_ptFilter_phi->Write();
        histEff_eta_ptFilter_charge->Write();
  outputFile->cd("/");  
 // outputFile->mkdir("Efficienza Filtro Iso");
 // outputFile->cd("Efficienza Filtro Iso");
  	histEff_IsoFilter_pt->Write();
        histEff_IsoFilter_eta->Write();
 	histEff_IsoFilter_eta_pt->Write();
  	histEff_IsoFilter_px->Write();
        histEff_IsoFilter_py->Write();
  	histEff_IsoFilter_pz->Write();
        histEff_IsoFilter_en->Write();
  	histEff_IsoFilter_phi->Write();
        histEff_IsoFilter_charge->Write();
  outputFile->cd("/");  
 // outputFile->mkdir("DiMuoni preTaglio Massa 130");
  //outputFile->cd("DiMuoni preTaglio Massa 130");
	hist_mass_inclusive_pre_MCut->Write();
	hist_p_inclusive_pre_MCut->Write();
	hist_pt_inclusive_pre_MCut->Write();
	hist_pz_inclusive_pre_MCut->Write();
	hist_eta_inclusive_pre_MCut->Write();
	hist_phi_inclusive_pre_MCut->Write();
  outputFile->cd("/");  
//  outputFile->mkdir("DiMuoni postTaglio Massa 130");
//  outputFile->cd("DiMuoni postTaglio Massa 130");
	hist_mass_inclusive_post_MCut->Write();
	hist_p_inclusive_post_MCut->Write();
	hist_pt_inclusive_post_MCut->Write();
	hist_pz_inclusive_post_MCut->Write();
	hist_eta_inclusive_post_MCut->Write();
	hist_phi_inclusive_post_MCut->Write();
  outputFile->cd("/");  
//  outputFile->mkdir("Jet");
//  outputFile->cd("Jet");
	hist_N_jet_btag_for_event->Write();
	hist_N_jet2_btag_for_event->Write();
	hist_bdisc->Write();
        hist_pt_MEt_btag_event->Write();
	hist_energy_MEt_btag_event->Write();
	hist_pt_MEt_non_btag_event->Write();
	hist_energy_MEt_non_btag_event->Write();
...
void NewAnalyzer_dacapo::Draw2() { // To be called After the Loop to look at plots
         TString sig_exists = "signal_" + sel_dataset + "tree";
         TString bkg_exists = "bkg_" + sel_dataset + "tree";
		 
		 TString file_name = "analysis.root";
...
void NewAnalyzer_dacapo::selectDimuonPair(std::vector<TLorentzVector> tight_muons, std::vector<bool> trigger, std::vector<int> charge, std::vector<TLorentzVector> &muon_pair){
...
TLorentzVector NewAnalyzer_dacapo::buildDimuonPair(std::vector<TLorentzVector> muon_pair,  double k1=1., double k2=1.){
...
void NewAnalyzer_dacapo::CleanJetsFromMuons(std::vector<TLorentzVector>& Jets, std::vector<float>& disc, std::vector<TLorentzVector> Muons, float angle) {
